(() => {
	...
	header.append(title, count, micButton)
	...
	function nextcloudParticipantIdForCard(card) {
		if (!isNextcloudRuntime || !(card instanceof Element)) return null
		return card.id.match(/^(?:placeholder-)?container_(.+)_video_incoming$/)?.[1] || null
	}

	function visibleCardArea(card) {
		const bounds = card.getBoundingClientRect()
		if (bounds.width <= 0 || bounds.height <= 0 || getComputedStyle(card).visibility === 'hidden') return 0
		return bounds.width * bounds.height
	}

	function findNextcloudParticipantCard(participantId) {
		if (!participantId) return null
		return [...document.querySelectorAll('.video-container[id]')]
			.filter((card) => nextcloudParticipantIdForCard(card) === participantId)
			.sort((left, right) => visibleCardArea(right) - visibleCardArea(left))[0] || null
	}

	function parkSource(source) {
		const previousCard = source.card
		source.card = null
		source.viewHost.dataset.placement = 'fallback'
		lanes.append(source.viewHost)
		restoreCard(previousCard)
		updateEmptyState()
	}

	function mountSource(source, element = null, explicitCard = null) {
		let nextCard = usesCardOverlays
			? (explicitCard?.isConnected ? explicitCard : participantCardFor(element))
			: null
		if (isNextcloudRuntime && source.direction === 'remote' && nextCard) {
			const candidateParticipantId = nextcloudParticipantIdForCard(nextCard)
			if (!source.nextcloudParticipantId && candidateParticipantId) {
				source.nextcloudParticipantId = candidateParticipantId
			}
			if (source.nextcloudParticipantId) {
				nextCard = findNextcloudParticipantCard(source.nextcloudParticipantId)
					|| (candidateParticipantId === source.nextcloudParticipantId ? nextCard : null)
			}
		}
		const previousCard = source.card
		if (nextCard) {
	...
	function reconcileNextcloudParticipantCard(source) {
		if (!isNextcloudRuntime || source.direction !== 'remote' || !source.nextcloudParticipantId) return
		const expectedCard = findNextcloudParticipantCard(source.nextcloudParticipantId)
		if (expectedCard) {
			if (source.card !== expectedCard || source.viewHost.parentElement !== expectedCard) {
				mountSource(source, null, expectedCard)
			}
			return
		}
		if (source.card && nextcloudParticipantIdForCard(source.card) !== source.nextcloudParticipantId) {
			parkSource(source)
		}
	}
	...
	function updateEmptyState() {
		...
		const hasMicTest = [...sources.values()].some((source) => source.origin === 'capture')
		const hasLocalSource = [...sources.values()].some((source) => source.direction === 'local' && source.origin !== 'capture')
		micButton.textContent = hasMicTest ? 'Stop mic test' : 'Mic test'
		micButton.disabled = micCapturePending || (hasLocalSource && !hasMicTest)
		micButton.title = hasMicTest
			? 'Stop microphone test'
			: hasLocalSource
				? 'Outgoing call audio is already detected'
				: 'Direct microphone fallback'
	}
	...
	const originalGetStats = peerConnectionPrototype?.getStats
	...
	const originalSenderGetStats = senderPrototype?.getStats
	const originalReceiverGetStats = receiverPrototype?.getStats
	let wrappedSetRemoteDescription = null
	let wrappedGetReceivers = null
	let wrappedGetSenders = null
	let wrappedGetStats = null
	let wrappedAddTrack = null
	let wrappedAddTransceiver = null
	let wrappedReplaceTrack = null
	let wrappedSenderGetStats = null
	let wrappedReceiverGetStats = null
	...
	if (!isJitsiRuntime && originalSetRemoteDescription && originalGetReceivers && originalGetSenders) {
		...
		wrappedSenderGetStats = function (...args) {
			captureLocalTrack(this.track)
			return originalSenderGetStats.apply(this, args)
		}
		wrappedReceiverGetStats = function (...args) {
			captureReceiverTrack(this.track)
			return originalReceiverGetStats.apply(this, args)
		}
		try {
			...
			if (originalGetStats) peerConnectionPrototype.getStats = wrappedGetStats
			...
			if (originalSenderGetStats) senderPrototype.getStats = wrappedSenderGetStats
			if (originalReceiverGetStats) receiverPrototype.getStats = wrappedReceiverGetStats
		} catch {}
	}
	...
	micButton.addEventListener('click', async () => {
		...
		const activeMicTests = [...sources.values()].filter((source) => source.origin === 'capture')
		if (activeMicTests.length) {
			for (const source of activeMicTests) removeSource(source.key)
			return
		}
		micCapturePending = true
		updateEmptyState()
		try {
			await resumeAudio()
			const stream = await originalGetUserMedia.call(mediaDevices, { audio: true, video: false })
			ownedStreams.add(stream)
			const source = addStream(stream, {
	...
	function destroy() {
		...
		if (peerConnectionPrototype?.getStats === wrappedGetStats) {
			try { peerConnectionPrototype.getStats = originalGetStats } catch {}
		}
		...
		if (senderPrototype?.getStats === wrappedSenderGetStats) {
			try { senderPrototype.getStats = originalSenderGetStats } catch {}
		}
		if (receiverPrototype?.getStats === wrappedReceiverGetStats) {
			try { receiverPrototype.getStats = originalReceiverGetStats } catch {}
		}
